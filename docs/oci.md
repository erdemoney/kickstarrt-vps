...
| **Networking → Primary VNIC** | select existing VCN `kickstarrt-vcn` and its **public subnet** (the one the wizard created); private IPv4 **automatically assigned**; **Public IPv4 address: Automatically assign** — the box gets a public IP for *outbound* internet only; ufw is deny-all, nothing listens inbound, so nothing is exposed. If the VCN dropdown is greyed out, set **Virtual cloud network compartment** to the root compartment (where `kickstarrt-vcn` was created) — the list only populates once that compartment is correct |
